//1. VARIABEL: Menyimpan data produk dalam bentuk JSON Array
const product = [
    { id: 1, nama: "Sofa Minimalis Biru", harga: 3200000, diskon: 4500000, kategori: "Ruang Tamu", stok: 5, gambar: "https://picsum.photos/id/1068/400/400"},
...
function renderProducts(){
    //LOOP: Menyeleksi elemen container
    ...
    products.forEach(product => {
        ...
        if (product.diskon > 0) {
            ...
            <span class="fw-bold text-danget">Rp 
            ${product.harga.toLocaleString()}</span>`;
        } else {
            priceHTML = `<span class = "fw-bold text-dark">Rp ${product.
            harga.toLocaleSring()}</span>`;
        }
        //Mengecek stok produk
        if (produk.stok > 0) {
            buttonHTML = `<button class="btn btn-buy p-2" onclick="alert
            ('Ditambahkan ke keranjang: ${product.nama}')">
            BeliSekarang</button>`;
        } else {
        ...
        const productHTML = `
            <div closs="col-12 col-md-6 col-lg-4 col-xl-3">
                <div class="card product-card shadow-sm p-2 position-relative">
                    ...
                    <div class="card-body">
                        ...
                        text-muted" stryle="font-size: 0.
                        7rem;">${product.kategori}</small>
                        <h6 class ="card-title fw-bold mb-3
                        met-1">${product.nama}</h6>
